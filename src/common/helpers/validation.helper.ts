...
export const validatePagination = (page: number, limit: number): void => {
  if (page < 1) {
    throw new BadRequestException('Invalid page number. It must be a positive integer greater than 0.');
  }

  if (limit < 1) {
    throw new BadRequestException('Invalid limit number. It must be a positive integer greater than 0.');
  }
};
